...
func NewQueueService(qr repository.QueueRepository, tr repository.TokenRepository, ar repository.AnalyticsRepository, rc *redis.Client, hub *websocket.Hub) *QueueService {
	return &QueueService{queueRepo: qr, tokenRepo: tr, analyticsRepo: ar, redis: rc, hub: hub}
}
...
func (s *QueueService) CallToken(tokenID uuid.UUID) (*models.Token, error) {
...
func (s *QueueService) CallNextToken(queueID uuid.UUID) (*models.Token, error) {
	token, err := s.tokenRepo.GetNextToken(queueID)
	if err != nil {
		return nil, errors.New("no tokens waiting")
	}
	return s.CallToken(token.ID)
}
