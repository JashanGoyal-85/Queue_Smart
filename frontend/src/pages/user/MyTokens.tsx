    ...
    <Layout breadcrumb="My Tokens">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div><p className="font-mono text-[9px] uppercase tracking-[0.18em] text-[#E85D32]">Queue history</p><h1 className="mt-1 text-3xl font-extrabold tracking-tight text-[#18201D]">My tokens</h1></div>
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          {STATUS_FILTERS.map(f => (
            <button key={f.value} onClick={() => setStatusFilter(f.value)}
              className={`px-4 py-2 rounded-full text-xs font-bold transition-colors ${statusFilter === f.value ? 'bg-[#18201D] text-white' : 'bg-white border border-black/10 text-black/55 hover:border-black/25'}`}>
              {f.label}
            </button>
          ))}
        </div>
